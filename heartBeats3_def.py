...
# define pi's name
    id="jimmy"
# kafka's ip and port
    heart="http://192.168.196.106:8082/topics/bpm"
...
# if __name__ == '__main__':
    ...
    while True:
        ...
        #print '-'*int(round(Signal*0.02, 0)), '|', Signal
        ...
        if N > 250 :                                   # avoid high frequency noise
            if  (Signal > thresh) and  (Pulse == False) and  (N > (IBI/5.0)*3.0)  :       
              ...
              BPM = int(round(60000/runningTotal, 0))               # how many beats can fit into a minute? that's BPM!)
              ...
              try:
                  ...
                  if BPM != "" :
                      ...
                      payload = {"records": [{"value": {"device_id": id, "timestamp": t, "bpm": str(BPM)}}]}
                      # send data to kafka
                      # r = requests.post(heart, data=json.dumps(payload), headers=headers)
                      # if link error
                     #  if r.status_code != 200:
                       #    print("Status Code(bpm): " + str(r.status_code))
                         #  print(r.text)
                      #else:
                          #print "bpm updated"
